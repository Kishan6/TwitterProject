...
fn2 = open('control.txt', 'r')

df2 = pd.DataFrame(columns= ['Tweet ID', 'tweet', 'Location', 'statuses', 'followers', 'friends', 'favourites', 'Date-time', 'class'])

cont2 = fn2.readlines() 
type(cont2)
#for i in range(0, 100, 11):
for i in range(0, len(cont2), 11):
    zero = cont2[i]
    one = cont2[i + 1]
    ...
    df2.loc[int((i/11) + counter)] = (zero, one, three, five, six, seven, eight, nine, 'control')
...
df2["statuses"] = pd.to_numeric(df2["statuses"])
df2["followers"] = pd.to_numeric(df2["followers"])
df2["friends"] = pd.to_numeric(df2["friends"])
df2["favourites"] = pd.to_numeric(df2["favourites"])
